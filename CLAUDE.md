# factor-desktop — Claude Code Context

> Electron + React 데스크탑 클라이언트. `anomaly-eye-monitor`(웹 FE) 와 같은 모노레포에 있지만 **화면이 다르다.**
> 작업 시작 전 root [`../CLAUDE.md`](../CLAUDE.md) 의 "코드 분리 룰북" / "새 페이지 추가 결정 트리" 섹션을 먼저 읽을 것.
...
전체 룰북(R1 ~ R10) 과 새 페이지 추가 결정 트리, Bad vs Good 예시는 root [`../CLAUDE.md`](../CLAUDE.md) 참조 — SoT 는 root.
