...
// 실측: 296px 미만에서는 text-xs semibold라도 "DASHBOA…"로 잘림.
// 기본은 여유 있게 320px로 둠.
        ...
        {customRecents ? (
          <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
            <div className="px-3 pt-3 pb-1 flex items-center justify-between flex-shrink-0">
              <h3 className="ui-caption font-medium">
                {recentsLabel}
              </h3>
            </div>
                ...
                {!isAuthenticated ? (
                  ...
                  <div className="space-y-3">
                    {grouped.map(({ group, items: gItems }) => (
                      <section key={group}>
                        <h4 className="px-2.5 py-1 text-xs font-medium text-muted-foreground/60 uppercase tracking-wider">
                          {group}
                        </h4>
...
function RecentsHeader({
  ...
  return (
    <div className="px-3 pt-3 pb-1 flex items-center justify-between flex-shrink-0">
      <h3 className="ui-caption font-medium">{label}</h3>
...
function EmptyState() {
  return (
    <div className="ui-caption py-8 text-center px-3">
      저장된 대화 없음
    </div>
  );
}
