  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto text-xs text-foreground/45 font-mono tabular-nums">
          {items.length}
      ...
      {!isLoading && !isError && items.length === 0 && (
        <div className="rounded-lg border border-border/40 bg-foreground/[0.02] px-6 py-12 text-center">
          ...
          <div className="ui-caption mt-1.5">
...
function ColHeader({ children }: { children: React.ReactNode }) {
  return (
    <th className="text-left px-3 py-2.5 text-xs uppercase tracking-[0.14em] text-muted-foreground/70 font-medium">
      {children}
    </th>
  );
}
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-3 align-top">
          <code className="block ui-fs-xs font-mono font-medium text-foreground break-all leading-tight">
            {spec.method_name}
          </code>
          <div className="ui-fs-xs text-foreground/45 mt-1 font-mono">
            {spec.adapter_type}
          </div>
        </td>
        ...
        <td className="px-3 py-3 align-top">
          <code className="block ui-fs-xs font-mono text-emerald-400/90 break-all leading-tight">
            {spec.path_template}
          </code>
          <div className="ui-fs-xs text-foreground/45 mt-1 font-mono tabular-nums truncate">
    ...
    <div className="px-7 py-6 space-y-7">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          type="button"
          onClick={onClose}
          className="text-xs text-foreground/55 hover:text-foreground transition-colors px-2 py-1 rounded hover:bg-foreground/[0.04]"
        >
          접기
        </button>
      </div>
      ...
      <section>
        ...
        <div className="mt-2 rounded-lg bg-foreground/[0.045] border border-border/30 px-4 py-3 flex items-center gap-3">
          <MethodTag method={spec.http_method} />
          <code className="flex-1 text-xs font-mono text-emerald-400/90 break-all">
            {spec.path_template}
          </code>
          <button
            type="button"
        ...
        <div className="mt-2 flex flex-wrap gap-4 ui-fs-xs text-foreground/55 font-mono">
          <span>
            timeout:{" "}
          ...
          {Object.keys(spec.query_params || {}).length > 0 ? (
            ...
            <>
              ...
              <div className="mt-2 rounded-md bg-foreground/[0.04] border border-border/30 px-3 py-2.5">
                <code className="text-xs font-mono text-foreground/85">
      ...
      {spec.notes && (
        <section>
          ...
          <p className="mt-2 ui-fs-xs text-foreground/80 leading-relaxed">
            {spec.notes}
          </p>
        </section>
      )}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground/75 font-medium">
      {children}
    </div>
  );
}
...
function JsonView({ value }: { value: unknown }) {
  return (
    <pre className="mt-2 text-xs font-mono bg-foreground/[0.04] border border-border/30 px-3 py-2.5 rounded-md text-foreground/85 overflow-x-auto leading-relaxed">
