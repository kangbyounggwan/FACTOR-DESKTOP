 ...
 *   · 섹션 헤더 (eyebrow) = `ui-fs-sm uppercase tracking-wider`
 *
    ...
    <aside className="w-56 flex-shrink-0 flex flex-col py-5 pl-10 pr-3">
      {desktopMenuGroups.map((group, gi) => (
        <div key={group.label ?? `g${gi}`} className={cn(gi > 0 && "mt-5")}>
          {group.label && (
            <p className="px-3 mb-1 ui-fs-sm uppercase tracking-wider text-muted-foreground/60 font-medium">
              {group.label}
            </p>
          )}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    "ui-fs-base rounded-md pl-3 pr-2.5 py-1.5",
                    active
