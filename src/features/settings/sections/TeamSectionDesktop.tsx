    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-3">
          <span className="text-[13px] text-foreground/45 font-mono tabular-nums">
            {filtered.length}
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size="sm" className="h-9 px-3 text-[14.5px]">
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-2">
                <div className="space-y-1.5">
                  <Label className="text-[13.5px] text-foreground/85 font-medium tracking-tight">
                    이메일 주소
                  </Label>
                  <Input
                    type="email"
                    ...
                    className="h-9 text-[14.5px] font-mono"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[13.5px] text-foreground/85 font-medium tracking-tight">
                  ...
                  >
                    <SelectTrigger className="h-9 text-[14.5px]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-3 min-w-0">
                        <Avatar className="h-8 w-8 flex-shrink-0 ring-1 ring-border/40">
                          ...
                          <AvatarFallback className="text-[12.5px] bg-foreground/[0.06] text-foreground/75 font-medium">
                            {(m.full_name || "??").slice(0, 2)}
                          </AvatarFallback>
                        </Avatar>
                        <div className="min-w-0">
                          <div className="text-[14.5px] text-foreground font-medium tracking-tight truncate">
                            {m.full_name || "이름 없음"}
                          </div>
                          <div className="text-[12.5px] font-mono text-foreground/55 truncate">
                            {m.email || "—"}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-2.5">
                      {m.status === "active" ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[13.5px] text-emerald-300">
                            활성
                          </span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[13.5px] text-amber-300">
                            대기중
                          </span>
                        </div>
                      )}
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <span className="font-mono tabular-nums text-[14px] text-foreground/85">
                        {m.equipmentCount}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-[13.5px] font-mono tabular-nums text-foreground/55">
...
function EmptyState({ search }: { search: string }) {
  return (
    <div className="rounded-lg border border-dashed border-border/50 bg-foreground/[0.015] px-6 py-12 text-center">
      ...
      <p className="text-[13.5px] text-muted-foreground/60 mt-1">
        {search
